...
/**
  ...
  */
static uint8_t ndef_uri[] = { 0x00, 0x17,                      // NDEF length (payload len+4)
                                    0xD1,                            // NDEF Header
                                    0x01,                            // NDEF type length
                                    0x13,                            // NDEF payload length  (url len+1)
                                    0x55,                            // NDEF Type
                                    0x02,                            // https://
								    // dorscluc.com/badge?b=xx
								    0x64, 0x6f, 0x72, 0x73,
								    0x63, 0x6c, 0x75, 0x63,
									0x2e, 0x6f, 0x72, 0x67,
									0x2f, 0x62, 0x61, 0x64,
...
__WEAK uint8_t *demoNdefFile = ndef_uri;
...
void demoCeInit( uint8_t* nfcfNfcid )
{
	...
	// update instance id in ndef
	char iid[10];
	snprintf(iid, sizeof(iid), "%02x", __INSTANCE_ID);
