...
static tag_handle_callback_t tag_handle_callback = NULL;
...
static void demoAPDU(cycle_return_t *ret);
static void demoNfcv( rfalNfcvListenDevice *nfcvDev );
static void demoNfcf( rfalNfcfListenDevice *nfcfDev );
static void demoT2t(cycle_return_t *ret);
static void demoCE( rfalNfcDevice *nfcDev, cycle_return_t *ret );
...
bool pollingDemoIni( void )
{
    ...
    // make the badge ID
    ceNFCF_nfcid2[2] = 0x48;
    ceNFCF_nfcid2[3] = 0x47;
    ceNFCF_nfcid2[4] = 0x44;
    ceNFCF_nfcid2[5] = 0x43;
    ceNFCF_nfcid2[6] = __INSTANCE_ID;
    ceNFCF_nfcid2[7] = (0x8f^ceNFCF_nfcid2[6])-1;

    // 0x5f 0x48 INSTANCE CS
    ceNFCA_NFCID[1] = 0x48;
    ceNFCA_NFCID[2] = __INSTANCE_ID;
    ceNFCA_NFCID[3] = (0x8f^ceNFCA_NFCID[2])-1;
...
void process_event(cycle_return_t *ret, rfalNfcDevice *dev, const char *type, nfc_proc_evt_t evttype){
	ret->event = true;
	ret->evt_type = evttype;
	const int idlen = dev->nfcidLen>32 ? 32 : dev->nfcidLen;
	memcpy(ret->id, dev->nfcid, idlen);
	ret->idlen = idlen;
	strncpy(ret->name, type, sizeof(ret->name));

	if(tag_handle_callback!=NULL){
		tag_handle_callback(ret);
	}
}
            ...
            if( rfalNfcIsDevActivated( rfalNfcGetState() ) )
            {
                ...
                switch( nfcDevice->type )
                {
                    ...
                    case RFAL_NFC_LISTEN_TYPE_NFCA:
                        ...
                        switch( nfcDevice->dev.nfca.type )
                        {
                            case RFAL_NFCA_T1T:
                                ...
                                process_event(&ret, nfcDevice, "ISO14443A/Topaz (NFC-A T1T)", NFC_PROC_EVT_SINGLE);
                                break;

                            case RFAL_NFCA_T4T:
                                //platformLog("NFCA Passive ISO-DEP device found. UID: %s\r\n", hex2Str( nfcDevice->nfcid, nfcDevice->nfcidLen ) );
                                process_event(&ret, nfcDevice, "NFCA Passive ISO-DEP", NFC_PROC_EVT_PRE);

                                demoAPDU(&ret);
                                process_event(&ret, nfcDevice, "NFCA Passive ISO-DEP", NFC_PROC_EVT_POST);
                                break;

                            case RFAL_NFCA_T4T_NFCDEP:
                            case RFAL_NFCA_NFCDEP:
                                //platformLog("NFCA Passive P2P device found. NFCID: %s\r\n", hex2Str( nfcDevice->nfcid, nfcDevice->nfcidLen ) );
                                process_event(&ret, nfcDevice, "NFCA Passive P2P", NFC_PROC_EVT_PRE);
                                demoP2P( nfcDevice );
                                process_event(&ret, nfcDevice, "NFCA Passive P2P", NFC_PROC_EVT_POST);
                                break;

                            default:
                                //platformLog("ISO14443A/NFC-A card found. UID: %s\r\n", hex2Str( nfcDevice->nfcid, nfcDevice->nfcidLen ) );
                                process_event(&ret, nfcDevice, "ISO14443A/NFC-A", NFC_PROC_EVT_PRE);
                                demoT2t(&ret);
                                process_event(&ret, nfcDevice, "ISO14443A/NFC-A", NFC_PROC_EVT_POST);
                                break;
                        }
                        break;
                    ...
                    case RFAL_NFC_LISTEN_TYPE_NFCB:
                        ...
                        process_event(&ret, nfcDevice, "ISO14443B/NFC-B", NFC_PROC_EVT_PRE);
                        if( rfalNfcbIsIsoDepSupported( &nfcDevice->dev.nfcb ) )
                        {
                            demoAPDU(&ret);
                        }
                        process_event(&ret, nfcDevice, "ISO14443B/NFC-B", NFC_PROC_EVT_POST);
                        break;
                    ...
                    case RFAL_NFC_LISTEN_TYPE_NFCF:
                        ...
                        if( rfalNfcfIsNfcDepSupported( &nfcDevice->dev.nfcf ) )
                        {
                            ...
                            process_event(&ret, nfcDevice, "NFCF P2P", NFC_PROC_EVT_PRE);
                            demoP2P( nfcDevice );
                            process_event(&ret, nfcDevice, "NFCF P2P", NFC_PROC_EVT_POST);
                        }
                        else
                        {
                            //platformLog("Felica/NFC-F card found. UID: %s\r\n", hex2Str( nfcDevice->nfcid, nfcDevice->nfcidLen ));
                        	process_event(&ret, nfcDevice, "Felica/NFC-F", NFC_PROC_EVT_PRE);
                            demoNfcf( &nfcDevice->dev.nfcf );
                            process_event(&ret, nfcDevice, "Felica/NFC-F", NFC_PROC_EVT_POST);
                        }
                    ...
                    case RFAL_NFC_LISTEN_TYPE_NFCV:
                        {
                            ...
                            process_event(&ret, nfcDevice, "ISO15693/NFC-V", NFC_PROC_EVT_PRE);
                            demoNfcv( &nfcDevice->dev.nfcv );
                            process_event(&ret, nfcDevice, "ISO15693/NFC-V", NFC_PROC_EVT_POST);
                        }
                        break;
                    ...
                    case RFAL_NFC_LISTEN_TYPE_ST25TB:
                        ...
                        process_event(&ret, nfcDevice, "ST25TB", NFC_PROC_EVT_SINGLE);
                        break;
                    ...
                    case RFAL_NFC_POLL_TYPE_AP2P:
                        ...
                        process_event(&ret, nfcDevice, "NFC P2P", NFC_PROC_EVT_PRE);
                        demoP2P( nfcDevice );
                        process_event(&ret, nfcDevice, "NFC P2P", NFC_PROC_EVT_POST);
                        break;
                    ...
                    case RFAL_NFC_POLL_TYPE_NFCF:
                        ...
                        if(nfcDevice->type==RFAL_NFC_POLL_TYPE_NFCA){
                        	process_event(&ret, nfcDevice, "CE NFCA mode", NFC_PROC_EVT_PRE);
                        }
                        else{
                        	process_event(&ret, nfcDevice, "CE NFCF mode", NFC_PROC_EVT_PRE);
                        }
                        demoCE(nfcDevice, &ret );

                        if(nfcDevice->type==RFAL_NFC_POLL_TYPE_NFCA){
                        	process_event(&ret, nfcDevice, "CE NFCA mode", NFC_PROC_EVT_POST);
                        }
                        else{
                        	process_event(&ret, nfcDevice, "CE NFCF mode", NFC_PROC_EVT_POST);
                        }
                        break;
...
static void demoCE( rfalNfcDevice *nfcDev, cycle_return_t *ret )
{
    ...
    do
    {
        ...
        switch( rfalNfcGetState() )
        {
            case RFAL_NFC_STATE_ACTIVATED:
                ...
                printf("RFAL got nfc state activated\r\n");
                break;
            ...
            case RFAL_NFC_STATE_DATAEXCHANGE_DONE:
                ...
                printf("RFAL got nfc state exchange\r\n");
                break;

            case RFAL_NFC_STATE_START_DISCOVERY:
                printf("RFAL got nfc state disc\r\n");
                return;

            case RFAL_NFC_STATE_LISTEN_SLEEP:
                printf("RFAL got nfc state sleep\r\n");
            default:
                break;
        }
    }
    ...
    const int dlen = *rcvLen>64 ? 64 : *rcvLen;
    ret->datalen = dlen;
    memcpy(ret->data, rxData, dlen);
...
static void demoT2t(cycle_return_t *ret)
{
    ...
    memcpy(ret->data, rxBuf, RFAL_T2T_READ_DATA_LEN);
    ret->datalen = RFAL_T2T_READ_DATA_LEN;
#endif
}
 ...
 *****************************************************************************
 */
void demoAPDU(cycle_return_t *ret)
{
#if RFAL_FEATURE_ISO_DEP_POLL
    ReturnCode err;
    uint16_t   *rxLen;
    ...
    const int dlen = *rxLen>64 ? 64 : *rxLen;
    ret->datalen = dlen;
    memcpy(ret->data, rxData, dlen);
